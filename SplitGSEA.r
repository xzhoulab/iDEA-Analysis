...
for(irpt in 1:20){
    ...
    #### generate GSEA files
    for(isplit in 1:2){
        load(paste0("~/H9vsDEC/split/Chu.H9vsDEC.split.test.Counts.rpt",irpt,".RData"))
        data = test_data[[isplit]]
        phenotype = data$cell_type
        count = data$counts
        load("~/Anotation/HaoA.GeneSets.c2.c5.c6.c7.larger20.RData")
        ...
        output.cls(pData,"phenotype", filename = paste0("~/H9vsDEC/GSEA/simPhenotype.split",isplit))
        #### generate count
        colnames(count) = c(paste0(1:length(phenotype),".CEL"))
        counts1 = cbind(genename,count)
        eset = createExpressionSet(pData,counts1)
        output.gct(eset,filename=paste0("~/H9vsDEC/GSEA/counts.rpt.",irpt,".split.",isplit,".RData"))
        #### generate Anno files
        for(iannot in 1:dim(Annotation)[2]){
            ...
            write.table(GoGene,file=paste0("~/H9vsDEC/GSEA/Anno.",iannot,"rpt.",irpt,".split.",isplit,".txt"),quote=F,row.names=T,col.names=F,sep="\t")
        }
    }
}
